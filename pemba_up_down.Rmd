...
```{r}
ggplot() +
  metR::geom_contour_fill(data = wind.data.monthly%>%filter(month == "Aug"), 
                          aes(x = lon, y = lat, z = speed), na.fill = TRUE, bins = 70) + 
  metR::geom_vector(data = wind.data.monthly%>%filter(month == "Aug"), 
...
ggplot() +
  metR::geom_contour_fill(data = wind.data.monthly%>%filter(month == "Jan"), 
                          aes(x = lon, y = lat, z = speed), na.fill = TRUE, bins = 70) + 
  metR::geom_vector(data = wind.data.monthly%>%filter(month == "Jan"), 
                    aes(x = lon, y = lat, dx = wind.x/10, dy = wind.y/10), 
                    arrow.angle = 30, arrow.type = "open", arrow.length = .75, 
                    pivot = 0,preserve.dir = TRUE, direction = "ccw")+
  geom_sf(data = tz.ke,fill = "lightgrey", col = "black")+
